...
import { Book, BookCondition, BookStatus } from './modules/books/schemas/book.schema';
...
async function bootstrap() {
  ...
  const catVanHoc = await categoryModel.create({ name: 'Văn học', slug: 'van-hoc', icon: '📑' });
  const catKinhTe = await categoryModel.create({ name: 'Kinh tế', slug: 'kinh-te', icon: '💰' });
  const catTamLy = await categoryModel.create({ name: 'Tâm lý - Kỹ năng', slug: 'tam-ly-ky-nang', icon: '✨' });
  const catThieuNhi = await categoryModel.create({ name: 'Sách thiếu nhi', slug: 'sach-thieu-nhi', icon: '👶' });
  ...
  await bookModel.create([
    {
      ...
      description: 'Moscow Edition hiếm sưu tầm bìa cứng. Còn rất mới không bị nhăn mép.'
    },
    {
    ...
    },
    {
      ...
      images: [{ url: 'https://images.unsplash.com/photo-1512820790803-83ca734da794?w=500', isMain: true }],
    ...
    },
    {
      ...
      images: [{ url: 'https://images.unsplash.com/photo-1626618012641-bfbca5a31239?w=500', isMain: true }],
    ...
    },
    {
      ...
      images: [{ url: 'https://images.unsplash.com/photo-1587876878363-22879586146c?w=500', isMain: true }],
