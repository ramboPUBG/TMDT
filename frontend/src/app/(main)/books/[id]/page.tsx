...
export default function BookDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: bookResult, isLoading, error: fetchError } = useFetch<ApiResponse<Book>>(`/books/${params.id}`);
  ...
  if (isLoading) {
    return <div className="py-20 text-center text-lg">Ðang t?i thông tin sách...</div>;
  }
    ...
    <div className="bg-muted/30 pb-16">
      <div className="bg-white border-b border-border mb-6">
        <div className="container mx-auto px-4 py-3 flex items-center gap-2 text-sm text-muted-foreground">
          <Link href="/" className="hover:text-primary">Trang ch?</Link>
          <span>/</span>
          {book.categoryId && (
             <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 flex flex-col gap-6">
            <div className="bg-white p-4 md:p-6 rounded-2xl border border-border flex flex-col md:flex-row gap-6">
              ...
              <BookInfo book={book} id={params.id} />
            </div>
            
            <div className="bg-white p-6 rounded-2xl border border-border">
              <h2 className="text-lg font-bold text-foreground mb-4 pb-2 border-b border-border">Mô t? s?n ph?m</h2>
              <div className="prose prose-sm max-w-none text-muted-foreground whitespace-pre-wrap">
                {book.description || 'Chua có mô t?'}
              </div>
              <div className="mt-6 pt-6 border-t border-border">
                <h3 className="text-sm font-bold text-foreground mb-3">Thu?c tính chi ti?t:</h3>
                <div className="grid grid-cols-2 gap-y-3 text-sm">
                  <div className="flex">
                    <span className="text-muted-foreground w-32">Danh m?c:</span>
                    <span className="font-medium text-foreground">{book.categoryId?.name}</span>
                  </div>
                  <div className="flex">
                     <span className="text-muted-foreground w-32">Ngày dang:</span>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-white p-5 rounded-2xl border border-border">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-foreground line-clamp-1">{book.sellerId?.fullName || "Ngu?i bán"}</h3>
                </div>
              </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-border">
              <h3 className="font-bold text-foreground mb-4">Cam k?t t? SachCu</h3>
              <ul className="flex flex-col gap-4 text-sm text-muted-foreground">
                <li className="flex items-start gap-3">
                  <div className="text-green-500 mt-0.5">???</div>
                  <div>
                    <p className="font-medium text-foreground">Hoàn ti?n 100%</p>
                    <p className="text-xs mt-1">N?u s?n ph?m không dúng nhu mô t?</p>
                  </div>
                </li>
              </ul>
            </div>
